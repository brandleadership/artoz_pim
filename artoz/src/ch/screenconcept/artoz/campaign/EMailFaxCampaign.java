...
public class EMailFaxCampaign extends GeneratedEMailFaxCampaign
{
				...
				final EMailCampaignParticipation participation) throws MailTemplateException
	{
		...
		final MailProxyFactory proxyFactory = DefaultMailProxyFactory.getInstance();// CampaignManager.getInstance().getMailProxyFactory();
		...
		String plainText = getPlainText() == null || getPlainText().trim().length() <= 20 ? null : evaluateWithAttribute( ctx, getPlainText(),
					"optOutInfo", insertOptOutLink( CampaignConfig.getCampaignConfig().getTextContent(), participation ) );
		String htmlText = getHtmlText() == null || getHtmlText().trim().length() <= 20 ? null : evaluateWithAttribute( ctx, getHtmlText(),
					"optOutInfo", insertOptOutLink( CampaignConfig.getCampaignConfig().getHtmlContent(), participation) );
		
		final Map attributes = new HashMap();
		attributes.put(EMail.RECIPIENT, recieverEMailAddress);
		attributes.put(EMail.SENDER, EMailConfig.evaluate(ctx, getSender()));
		attributes.put(EMail.REPLYTO, null);
		attributes.put(EMail.SUBJECT, EMailConfig.evaluate(ctx, getSubject()));
		attributes.put(EMail.TYPE, CampaignManager.getInstance().getCampaignConfig("eMailCampaign"));
		attributes.put(EMail.USER, user);
        attributes.put(EMail.TEXT, plainText);
        attributes.put(EMail.TEXTASHTML, htmlText);
		final EMail email = CampaignManager.getInstance().createEMail(attributes);
	...
	private String insertOptOutLink( final String template, final EMailCampaignParticipation participation )
	{
		...
		if (participation == null)
			...
			link = getFrontEndHostAndPort()+"microspot/__HYBRIS__/newsLetterUnregistration.jsf";
			//link = CampaignTools.buildFrontendLink( getFrontEndHostAndPort(), "optOut.do", Collections.singletonList( ( new StringBuilder(
			//"participation=" ) ).append( participation.getPK().toString() ).toString() ) );
		return MessageFormat.format( template, new Object[] { link } );
	}
...
// if (isFaxFaxOnly() || recieverEMailAddress.equals(""))
// {
// try
// {
// attributes.put(EMail.RECIPIENT, getFaxNumber(user));
...
// attributes.put(EMailFax.TYPE,
// CampaignManager.getInstance().getCampaignConfig("eMailCampaign"));
// attributes.put(EMail.USER, user);
